...
`~/.claude/plugins/config/claude-for-legal/employment-legal/CLAUDE.md` → jurisdictional footprint, hiring review triggers, restrictive covenant policy, offer letter template location.
...
Check the jurisdiction table in `~/.claude/plugins/config/claude-for-legal/employment-legal/CLAUDE.md` for this location. If it's not in the table — first hire in this jurisdiction — flag it: "First hire in [location]. The jurisdiction table doesn't cover this. Research needed before the offer goes out."
...
Per `~/.claude/plugins/config/claude-for-legal/employment-legal/CLAUDE.md` restrictive covenant policy: does this role require restrictions, and if so, at what scope? Apply the house policy first, then overlay the research.
...
**Before producing a "Clear to send" recommendation or a final offer for signature:** Read `## Who's using this` in `~/.claude/plugins/config/claude-for-legal/employment-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
